...
class Tataw3_c extends MY_Controller {
	public function __construct()
	{
        ...
        $this->load->model('tataw3/Tataw3_m');
               /**********************************************************/ 
    ...
    private  function upload_image($file_name){
        $config['upload_path'] = 'uploads/tato3/tato3_logo';
    ...
    public function read_file()
    {
        ...
        $file_path = 'uploads/tato3/tato3_logo'.$file_name;
    ...
    {
        $this->load->helper('file');
        $file_path ='uploads/tato3/morfqat/'.$file_name;
        header('Content-Type: application/pdf');
        header('Content-Discription:inline; filename="'.$file_name.'"');
        header('Content-Transfer-Encoding: binary');
        header('Accept-Ranges:bytes');
	...
	public function new_motato3()//tataw3/Tataw3_c/new_motato3
    {
        if($this->input->post('add')){
            ...
            messages('success','إضافة  متطوع');
                redirect('tataw3/Tataw3_c/new_motato3','refresh');
        }
       ...
       $data['reasons'] = $this->Tataw3_m->select_settings_tat_reason(101);
        ...
        $data['subview'] = 'admin/tataw3_v/motat3en/new_motato3';
    ...
    public function add_confirm()
{
    ...
    {    
        ...
        $data['subview'] = 'admin/tataw3_v/motat3en/motato3enConfirm';
        $this->load->view('admin_index', $data);
    }
    /////yara_tato3
    public function load_tahwel()
    {
            ...
            if(isset($id)&&!empty($id))
        {
            ...
            $this->load->view('admin/tataw3_v/motat3en/load_frd',$data);
        } elseif ($type == 2) {
            $data['talb_num']=$this->Tataw3_m->get_last_id();
            if(isset($id)&&!empty($id))
        {
        ...
        $data['work_type'] = $this->Tataw3_m->get_table('tat_settings',array('from_code'=>601));
            $this->load->view('admin/tataw3_v/motat3en/load_geha',$data);
        } 
    }
    public function load_reason(){
      ...
      $this->load->view('admin/tataw3_v/motat3en/load_reasons', $data);
}
public function get_details()
{
    ...
    $data['volunteer_detail_field'] = $this->Tataw3_m->getRecordfieldById(array('tat_id_fk'=>$id));
    $data['volunteer_work_type'] = $this->Tataw3_m->getRecordreasonById_type(array('tat_id_fk'=>$id));
    $this->load->view('admin/tataw3_v/motat3en/detail_page', $data);
}
public function add_morfqat($id)
{
    ...
    $data['subview'] = 'admin/tataw3_v/motat3en/morfqat_motato3';
    $this->load->view('admin_index', $data);
}
...
{
    $config['upload_path'] = 'uploads/tato3/morfqat';
    $config['allowed_types'] = 'gif|Gif|ico|ICO|jpg|JPG|jpeg|JPEG|BNG|png|PNG|bmp|BMP|WMV|wmv|MP3|mp3|FLV|flv|SWF|swf|pdf|PDF|xls|xlsx|mp4|doc|docx|txt|rar|tar.gz|zip';
...
public function load()
    ...
    $data['morfqat'] = $this->Tataw3_m->get_table('tat_morfq',array('tat_id_fk'=>$id));
    $data['volunteer'] = $this->Tataw3_m->getRecordById(array('id'=>$id));
         $this->load->view('admin/tataw3_v/motat3en/load', $data);
     }
}