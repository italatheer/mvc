...
class Tazkra_orders extends MY_Controller {
    public function __construct(){
        ...
        $this->load->model('technical_support/tazkra_orders/Tazkra_orders_model');
    ...
    public function add_order(){ // technical_support/tazkra_orders/Tazkra_orders/add_order
        ...
        if ($this->input->post('add')){
           ...
           $time = $full_date[1];
            ...
            $this->messages('success',' تم انشاء التذكرة رقم ' .' '.$tazkra_num.' '. 'بتاريخ  '.' '.$new_date.' ' .$time);
            redirect('technical_support/tazkra_orders/Tazkra_orders/add_attach/'.$id,'refresh');
        }
        $data['title']='   اضافة تذكرة جديدة';
        $data['subview']= 'admin/technical_support/tazkra_orders/tazkra_orders_view';
    ...
    public function update_order($id){
       ...
       // $data['all_tzaker']= $this->Tazkra_orders_model->get_all_tazaker();
        ...
        if ($this->input->post('add')){
            ...
            redirect('technical_support/tazkra_orders/Tazkra_orders/add_order','refresh');
        }
        $data['title']='   اضافة تذكرة جديدة';
        $data['subview']= 'admin/technical_support/tazkra_orders/tazkra_orders_view';
        $this->load->view('admin_index',$data);
    ...
    public function add_attach($id){
        ...
        if ($this->input->post('add')){
            ...
            $this->Tazkra_orders_model->add_attach($id,$file);
            $this->messages('success',' تم اضافة المرفق بنجاح');
            redirect('technical_support/tazkra_orders/Tazkra_orders/add_attach/'.$id,'refresh');
        }

        $data['title']='   اضافة تذكرة جديدة   ';
        $data['subview']= 'admin/technical_support/tazkra_orders/add_attach';
        $this->load->view('admin_index',$data);
    }
    public function load_details(){
       ...
       // $this->test($data['get_all']);
        $this->load->view('admin/technical_support/tazkra_orders/load_details',$data);
    ...
    public function  delete_attach($id,$tzkra_id){
        ...
        $this->messages('success',' تم الحذف بنجاح');
        redirect('technical_support/tazkra_orders/Tazkra_orders/add_attach/'.$tzkra_id,'refresh');
    }
    public function Print_order(){

        $row_id = $this->input->post('row_id');
        $data['get_all']=$this->Tazkra_orders_model->get_tzkra_byId($row_id);
        $this->load->view('admin/technical_support/tazkra_orders/print_order', $data);
