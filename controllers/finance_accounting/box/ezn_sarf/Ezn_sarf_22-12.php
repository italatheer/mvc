...
public function getConnection($Fe2aType)
    {
         ...
         *           $row_employee['emp_code'],
                                ...
                                $row_employee['administration'],
                                $row_employee['department']*/
        /********* خاص بالموظفين*********************************************/
        if($Fe2aType == 1 ){
            ...
            if(!empty($all_employees)){
                foreach($all_employees as $row_employee){
                    ...
                    $all_employees['data'][] = array(
                        '<input type="radio" name="choosed" value="'.$row_employee['id'].'"
            ...
            if(!empty($all_Basic)){
                foreach($all_Basic as $row_Basic){
                    ...
                    $all_Basic['data'][] = array(
                        '<input type="radio" name="choosed" value="'.$row_Basic['id'].'"
                         ondblclick="GetMemberName('.$row_Basic['id'].')"   id="member'.$row_Basic['id'].'" data-name="'.$row_Basic['father_name'].'" data-id="'.$row_Basic['id'].'"
                        onclick="getMemberData($(this).val(),'.$Fe2aType.')" />',
                        $row_Basic['file_num'],
                    ...
                    $arr_general_assembly['data'][] = array(
                        '<input type="radio" name="choosed" value="'.$row_general_assembly->id.'"
                          ondblclick="GetMemberName('.$row_general_assembly->id.')"   id="member'.$row_general_assembly->id.'" data-name="'.$row_general_assembly->title.'" data-id="'.$row_general_assembly->id.'"
                                data-mob=""    onclick="getMemberData($(this).val(),'.$Fe2aType.')" />',
...
/*
    ...
    */
    ...
    public function GetDeviceData(){
        ...
        if($_POST['type'] === 'bank'){
            $data =$this->Ezn_sarf_model->select_all_by_DeviceData(array('fr_devices_points.device_id_fk'=>$_POST['id']),'bank_id_fk');
        }elseif ($_POST['type'] === 'Account'){
            $data =$this->Ezn_sarf_model->select_all_by_DeviceData(
    ...
    public  function DeletePill($id){
        $this->Ezn_sarf_model->DeletePill($id);
    ...
    public function GetDetails(){
        ...
        $data['all_banks'] = $this->Ezn_sarf_model->select_all_by_condition(array("society_main_banks_account.type"=>2)
            ,"society_main_banks_account.bank_id_fk");

        $data['erad_tbro3_arr'] = $this->Ezn_sarf_model->select_fr_bnod_pills_setting_by_condition(
            array('fe2a'=>0,'band'=>0,'erad_tabro3'=>0,'esal'=>$data['result']->pill_type));

        $data['fe2a_type_arr'] = $this->Ezn_sarf_model->select_fr_bnod_pills_setting_by_condition(
            array('fe2a'=>0,'band'=>0,'erad_tabro3'=>$data['result']->erad_type));

        $data['bnd_type_arr'] = $this->Ezn_sarf_model->select_fr_bnod_pills_setting_by_condition(
            array('band'=>0,'fe2a'=>$data['result']->fe2a_type1));



        $data['fe2a_type_arr2'] = $this->Ezn_sarf_model->select_fr_bnod_pills_setting_by_condition(
            array('fe2a'=>0,'band'=>0,'erad_tabro3'=>$data['result']->erad_type));

        $data['bnd_type_arr2'] = $this->Ezn_sarf_model->select_fr_bnod_pills_setting_by_condition(
            array('band'=>0,'fe2a'=>$data['result']->fe2a_type2));

        $data['fe2a_types'] = $this->Difined_model->select_all('fr_sponser_donors_setting','','',"","");
        $data['bank_accounts_arr'] =$this->Ezn_sarf_model->select_all_by_condition(
            array('society_main_banks_account.bank_id_fk'=>$data['result']->bank_id_fk),'');

        $data['bank_account_num_arr'] =$this->Ezn_sarf_model->select_all_by_condition(
        ...
        $data['shabka_banks_arr'] =$this->Ezn_sarf_model->select_all_by_DeviceData(
            array('fr_devices_points.device_id_fk'=>$data['result']->device_num),'bank_id_fk');

        $data['bank_account_shabka_arr'] =$this->Ezn_sarf_model->select_all_by_DeviceData(
            array('fr_devices_points.device_id_fk'=>$data['result']->device_num,
                'fr_devices_points.bank_id_fk'=>$data['result']->bank_id_fk),'');



        $data['bank_account_num_shabka_arr'] =$this->Ezn_sarf_model->select_all_by_DeviceData(
            array('fr_devices_points.account_id_fk'=>$data['result']->bank_account_num
            ,'fr_devices_points.bank_id_fk'=>$data['result']->bank_id_fk,
                'fr_devices_points.device_id_fk'=>$data['result']->device_num),'');

        $data['esal_type'] = $this->Ezn_sarf_model->select_fr_bnod_pills_setting_by_condition(array('esal'=>0));
        $data['gathering_place'] =  $this->Ezn_sarf_model->GetFromFr_settings(14);
        $data['titles'] = $this->Ezn_sarf_model->GetFromFr_settings(8);
        $data['greetings'] = $this->Ezn_sarf_model->GetFromFr_settings(9);
...
public function deleteEzanSarf($id)
    {
        ...
        $this->message('success','حذف  بيانات الإيصال');
        redirect('finance_accounting/box/ezn_sarf/Ezn_sarf/addEzn','refresh');
    }
