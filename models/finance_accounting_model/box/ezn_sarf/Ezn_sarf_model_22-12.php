...
public function getFinanceEznSarf()
	{
		...
		if ($sql->num_rows() > 0) {
			...
			foreach ($sql->result() as $row) {
				$data[$x] = $row;
				if ($row->type == 1) {
					$data[$x]->Pname = $this->getPname($row->p_id_fk,'employees','employee');
				}
				if ($row->type == 2) {
					$data[$x]->Pname = $this->getPname($row->p_id_fk,'basic','father_name');
				}
				if ($row->type == 3) {
					$data[$x]->Pname = $this->getPname($row->p_id_fk,'finance_gehat','title');
				}
				$x++;
			}
			return $data;
	...
	public function getPname($id,$table,$field)
	{
		return $this->db->select($field)->where('id',$id)->get($table)->row_array()[$field];
	}
