...
class Solaf_requests_model extends CI_Model
{
    ...
    public function insert_solfa()
    {
        ...
        $this->db->insert('hr_solaf', $data);
        if(   $this->input->post('qust_num')>0)
        {
            $x= $this->input->post('qust_num');
for($i=0;$i<$x;$i++)
{
    $dataw['t_rkm_fk'] = $this->input->post('t_rkm');
    $dataw['emp_code_fk'] = $this->input->post('emp_code_fk');
    $dataw['value_of_quest'] =  $this->input->post('qemt_solaf')/ $this->input->post('qust_num');
...
for($i=0;$i<$x;$i++)
{
    $dataw['t_rkm_fk'] = $this->input->post('t_rkm');
    $dataw['emp_code_fk'] = $this->input->post('emp_code_fk');
    $dataw['value_of_quest'] = round(( $this->input->post('qemt_solaf')/ $this->input->post('qust_num'))*10)/10;
    $this->db->where('t_rkm_fk', $id)->update('hr_solaf_quest', $dataw);
}
...
/*	public function get_data()
    {
     ...
     $data['t_rkm_date_m'] = $this->input->post('t_rkm_date_m');
        ...
        $data['qust_num'] = $this->input->post('qust_num');
        $data['khsm_form_date_h'] = $this->input->post('khsm_form_date_h');
