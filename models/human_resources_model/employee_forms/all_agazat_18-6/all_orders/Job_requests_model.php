...
public function select_marad(){

    $query = $this->db->get('hr_marad');
    if ($query->num_rows() > 0) {
        return $query->result() ;
    }
    return false;
...
public function insert_marad(){
    $data['marad_name'] = $this->input->post('title');
 
    $this->db->insert('hr_marad', $data);
}

public function get_marad_by_id($id){
    $this->db->where('id',$id);
    $query = $this->db->get('hr_marad');
    if ($query->num_rows() >0){
        return $query->row();
...
public function update_marad($id){


    $data['marad_name']= $this->input->post('marad_title');
  
    $this->db->where('id',$id);
    $this->db->update('hr_marad',$data);
}
public function delete_marad($id){

    $this->db->where('id',$id);
    $this->db->delete('hr_marad');
}
