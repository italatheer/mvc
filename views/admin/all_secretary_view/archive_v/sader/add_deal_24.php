                        ...
                        <div class="col-sm-9 col-xs-12">
                            ...
                            <table class="table  table-striped table-bordered dt-responsive nowrap">
                                ...
                                <tr>
                                    ...
                                    <?=$get_all->year.'/'. $get_all->emp_depart_code .'/'.$get_all->sader_rkm?>
                                    </span>
                                    </td>
...
<!-- </div>-->
      ...
      <?php }?>
               ...
               <div class="col-xs-4 no-padding" style="height: 150px; overflow-y: scroll;">
                       <h5>:قائمة المستلمين</h5>
...
<div class="reply-comment ">
    <h5 class="name"><span class="label label-inverse"><?= $comment->publisher_name?> </span>
    <span class="label" style="background-color: #32e26b">  <?php echo date('h:i A',$comment->ttime);?> </span>
        ...
        <div class="btn-group" role="group" aria-label="..." style="float:left;">
            ...
            if ($_SESSION['role_id_fk'] ==1){
                ...
                <button type="button" class="btn btn-default"  onclick="get_comment(<?= $comment->id?>,'<?= $comment->comment?>','<?= $comment->to_user_id?>','<?= $comment->to_user_name?>')"><i class="fa fa-pencil"> </i></button>
                <?php
            } else{
...
?>
                ...
                <div class="tab-pane active" id="pag6" role="tabpanel">
...
<script>
    function insert_comments(sader_id) {
        ...
        var to_user_name = [];
    $("input[name='to_twgeh_user_fk_name']").each(function(){
        to_user_name.push(this.value);
        });
        var to_user_id = [];
    $("input[name='to_twgeh_user_fk']").each(function(){
       ...
       // alert(sader_id);
        if (comment !='' && to_user_name !=''){
            $.ajax({
                ...
                data: {sader_id:sader_id,comment:comment,row_id:row_id,to_user_id:to_user_id,to_user_name:to_user_name},
                dataType: 'html',
                cache: false,
                success: function (html) {
                    if (row_id !=''){
...
<script>
     function get_comment(id,value,to_user_id,to_user_name) {
        $('#comment').val(value);
        $('#comment_id').val(id);
        $('#to_twgeh_n').val(to_user_name);
        $('#to_twgeh_n').append("<input type='hidden'  data-validation='required' id='to_twgeh_user_fk_name"+to_user_id+"' name='to_twgeh_user_fk_name' value='"+to_user_name+"'/>");
