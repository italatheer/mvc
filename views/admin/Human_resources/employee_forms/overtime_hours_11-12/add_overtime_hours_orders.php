...
<?php
if (isset($result_page) && !empty($result_page)) {
    $order_num = $result_page->order_num;
    $order_date = $result_page->order_date_ar;
    $emp_id_fk = $result_page->emp_id_fk;
    $emp_name = $result_page->emp_name;
    $emp_code = $result_page->emp_code_fk;
    $emp_national_num = $result_page->emp_national_num;
    $mosama_wazefy = $result_page->mosama_wazefy;
    $edara_id_fk = $result_page->edara_id_fk;
    $edara_n = $result_page->edara_n;
    $qsm_id_fk = $result_page->qsm_id_fk;
    $qsm_n = $result_page->qsm_n;
    $direct_manager_id_fk = $result_page->direct_manager_id_fk;
    $direct_manger_code = $result_page->direct_manger_code;
    $direct_manger_n = $result_page->direct_manger_n;
    $direct_manger_mosama_wazefy = $result_page->direct_manger_mosama_wazefy;
    $nazran_to = $result_page->nazran_to;
    $work_assigned_details = $result_page->work_assigned_details;
    $total_hours = 0;
    if (isset($result_page->details) && !empty($result_page->details)) {
        foreach ($result_page->details as $item) {
            $total_hours += $item->num_hours;
        }
    }
    // $total_hours= $result_page->total_hours;
...
<div class="col-sm-12 no-padding ">
    ...
    <div class="panel panel-bd lobidisable lobipanel lobipanel-sortable ">
        ...
        <div class="panel-body">
            <?php
            if (isset($result_page) && !empty($result_page)){ ?>
            <form action="<?php echo base_url(); ?>human_resources/employee_forms/overtime_hours/Overtime_hours_orders/edit_overtime_hours_orders/<?php echo $result_page->id; ?>"
                  method="post">
                <?php } else{ ?>
                                        ...
                                        if (isset($result_page->details) && !empty($result_page->details)) {
                                            ...
                                            foreach ($result_page->details as $item) {
                                                ...
                                                <tr class="open_green" id="tr<?= $z ?>">
                                                    ...
                                                    <td><input type="time"" class="form-control "
                                                        ...
                                                        name="from_hour[]" id="from_time<?= $z ?>"
                                                        onchange="count(<?= $z ?>)">
                                                    </td>
                                                    <td><input type="time"" class="form-control "
                                                        data-validation="required" value="<?= $item->to_hour ?>"
                                                        name="to_hour[]" id="to_time<?= $z ?>"
                                                        onchange="count(<?= $z ?>)" >
                                                    </td>
                                            ...
                                            <tr class="open_green" id="tr1">
                                                ...
                                                <td><input type="time"" class="form-control " data-validation="required"
                                                    name="from_hour[]" id="from_time1"
                                                    onchange="count(1)">
                                                </td>
                                                <td><input type="time"" class="form-control " data-validation="required"
                                                    name="to_hour[]" id="to_time1"
                                                    onchange="count(1)">
                                                </td>
                                                <td><input type="text" class="num_hours calc form-control"
