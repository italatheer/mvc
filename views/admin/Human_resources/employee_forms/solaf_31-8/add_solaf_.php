    ...
    <div class="col-sm-12 no-padding ">
        ...
        <div class="panel panel-bd lobidisable lobipanel lobipanel-sortable ">
            ...
            if (isset($item) && !empty($item)) {
                $rkm=$item->t_rkm ;
                $t_rkm_date_h=$item->t_rkm_date_h;
                $t_rkm_date_m=$item->t_rkm_date_m;
                $qemt_solaf=$item->qemt_solaf;
                $sadad_solfa=$item->sadad_solfa;
                $qust_num=$item->qust_num;
                $khsm_form_date_h=$item->khsm_form_date_h;
                $khsm_form_date_m=$item->khsm_form_date_m;
                $previous_request_date_h=$item->previous_request_date_h;
...
<!--
      ...
      -->          
                                ...
                                <div class="form-group col-md-2 col-sm-6 col-xs-6 padding-4">
                    <label class="label">  طريقة سداد السلفة  </label>
                    <select name="sadad_solfa" id="sadad_solfa" value="<?php if($sadad_solfa==1){echo 'دفع نقدا';}elseif($sadad_solfa==2){echo ' تخصم مرة واحدة من الراتب';}
                    elseif($sadad_solfa==3){echo 'تخصم شهريا من الراتب';}
                    ?>"
                            class="form-control" style="    width: 190px;
    height: 33px;"
                            data-show-subtext="true" data-live-search="true" aria-required="true">
                        <option value="<?= $sadad_solfa?>"><?php if($sadad_solfa==1){echo 'دفع نقدا';}elseif($sadad_solfa==2){echo ' تخصم مرة واحدة من الراتب';}
                        elseif($sadad_solfa==3){echo 'تخصم شهريا من الراتب';}
                        else{echo 'اختر';}?></option>
                       
                                <option  value="1" > دفع نقدا</option>
                                <option  value="2">  تخصم مرة واحدة من الراتب</option>
                                <option  value="3">  تخصم شهريا من الراتب</option>
    ...
    <input type="text" name="qust_num" id="qust_num"
           value="<?php echo $qust_num; ?>"
           class="form-control "
...
<input type="text" 
       value="<?php if($qust_num>0){echo $qemt_solaf/$qust_num; }?>"
       class="form-control "
       data-validation="required" onkeypress="validate_number(event);"
       data-validation-has-keyup-event="true">
                        ...
                        foreach ($items as $row) {
                            ...
                            <tr>
                                <td><?php echo $x; ?></td>
                                 ...
                                 <td></td>
                                   <td>وليد عبدالمجيد عبدالوهاب سراج الدين	</td>
                                <td><?php echo $row->qemt_solaf; ?></td>
                                <td><?php if($row->sadad_solfa==1){echo 'دفع نقدا';}elseif($row->sadad_solfa==2){echo 'تخصم مرة واحدة من الراتب';}
                    elseif($row->sadad_solfa==3){echo 'تخصم شهريا من الراتب';}
                    ?></td>
                    <td><?php echo $row->qust_num; ?></td>
                                <td><?php if($row->qust_num>0){echo $row->qemt_solaf/$row->qust_num; }?></td>
                                <td><?php echo $row->khsm_form_date_m; ?></td>
                                <td>
                                  

<!-- <a href="<?php echo base_url(); ?>human_resources/employee_forms/solaf/Solaf/edit_solaf/<?php echo $row->id; ?>"><i
        ...
        function(){
        window.location="<?= base_url() . 'human_resources/employee_forms/solaf/Solaf/edit_solaf/' .  $row->id ?>";
        });'>
    <i class="fa fa-pencil"> </i></a>
<!-- <a href="<?php echo base_url(); ?>human_resources/employee_forms/solaf/Solaf/delete_solfa/<?php echo $row->id; ?>"
onclick="return confirm('هل انت متاكد  من عمليه الحذف؟');"><i class="fa fa-trash"
        ...
        setTimeout(function(){window.location="<?= base_url() . 'human_resources/employee_forms/solaf/Solaf/delete_solfa/' . $row->id ?>";}, 500);
        });'>
