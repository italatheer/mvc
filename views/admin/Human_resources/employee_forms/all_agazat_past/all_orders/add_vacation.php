        ...
        <div class="panel panel-bd lobidisable lobipanel lobipanel-sortable ">
            ...
            <div class="panel-body">
                <div class="col-sm-12 no-padding">
                    ...
                    if (isset($item) && !empty($item)){
                    ?>
                    <form action="<?php echo base_url() . 'human_resources/employee_forms/all_agazat/all_orders/Vacation/edit_vacation/' . $item->id; ?>"
                          method="post" id="form1">

                        <?php } else{ ?>
                        <form action="<?php echo base_url(); ?>human_resources/employee_forms/all_agazat/all_orders/Vacation/add_vacation"
                              method="post" id="form1">
...
l bottom-input"
    ...
    <input type="text" name="marad_name" id="marad_name" value="<?= $marad_name ?>"
    ...
    <i class="fa fa-plus"></i></button>
                                ...
                                <div class="col-md-12 no-padding" id="mowazf_badel_div" style="display: none">
                                    ...
                                    <div class="bond panel-default">
                                        ...
                                        <div class="bond-body">
                                            <div class="form-group col-md-12 col-sm-12 col-xs-12 padding-4">
                                                <label> <input type="checkbox" name="pledge" id="pledge" value="1"  data-validation="required" >
                                                    اتعهد بتسليم
                                                    كل
...
<!--
                <div class="col-sm-2">
                    <div class="personel-img">
                        <div class="front">
                            <img id="personel_img_src" src="<?php echo base_url() ?>asisst/admin_asset/img/logo.png"/>
                        </div>
                        <div class="back">
                            <div class="show-da">
                   ...
                   <div class="col-sm-12 form-group">
                       <div class="col-sm-3  col-md-3 padding-4 ">
                           ...
                           <button type="button" class="btn btn-labeled btn-success " onclick="$('#geha_input').show(); show_add()"
                               ...
                               <span class="btn-label"><i class="glyphicon glyphicon-plus"></i></span>إضافة مرض
                           </button>
                   ...
                   <div class="col-sm-12 no-padding form-group">
                       ...
                       <div id="geha_input" style="display: none">
                           <div class="col-sm-3  col-md-5 padding-2 ">
                               <label class="label label-green  ">إسم المرض </label>
                               <input type="text" name="marad_title" id="marad_title"
                                      value=""
                           ...
                           <div class="col-sm-3  col-md-2 padding-4" id="div_add" style="display: block;">
                               <button type="button" onclick="add_marad($('#marad_title').val())" style="    margin-top: 27px;"
               ...
               <div id="myDiv_geha"><br><br>
                        <?php if (!empty($marad_table)) { ?>
                            <table id="" class=" example table table-bordered table-striped" role="table">
                                ...
                                <?php
                                $x = 1;
                                foreach ($marad_table as $value) {
                                    ?>
                                    <tr>
                                        <td><input type="radio" name="radio" data-title="<?= $value->id ?>"
                                                   id="radio" ondblclick="getTitle($(this).attr('data-title'),'<?php echo $value->marad_name;?>')"></td>
...
-->
                                            ...
                                            <a href="#" onclick="update_marad(<?= $value->id?>,'<?= $value->marad_name?>');"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>
                            ...
                            <tr>
                                ...
                                <td>
                                    <button type="button" class="btn btn-info btn-xs" data-toggle="modal"
                                            data-target="#myModal<?php echo $row->id; ?>">التفاصيل
                                    </button>
                                </td>
                                ...
                                <td>
                                    ...
                                    <a href="<?php echo base_url(); ?>human_resources/employee_forms/all_agazat/all_orders/Vacation/edit_vacation/<?php echo $row->id; ?>"><i
                                                class="fa fa-pencil-square-o" aria-hidden="true"></i> </a>

                                    <a href="<?php echo base_url(); ?>human_resources/employee_forms/all_agazat/all_orders/Vacation/delete_vacation/<?php echo $row->id; ?>"
...
<script>
   ...
   /* function checkValidateMaxMin() {
        if (parseFloat($('#num_days').val()) <= parseFloat($('#max_days').val()) && parseFloat($('#num_days').val()) >= parseFloat($('#min_days').val())) {
            ...
            Swal.fire({
                title:  'المدة المحددة لا يجب أن تخطت أقصى عدد من الأيام  أو أقل عدد مسموح بها',
                html: ' اقصى عدد :<strong>'+$('#max_days').val() +'</strong>  ' +
                    '<br>' +
                    ' اقل عدد :<strong>'+$('#min_days').val() +'</strong>  ',
                type: 'warning',
                confirmButtonColor: '#3085d6',
...
<script>
    function get_emp_data(valu) {
        ...
        $.ajax({
            type: 'post',
            url: "<?php echo base_url();?>human_resources/employee_forms/all_agazat/all_orders/Vacation/get_load_page",
            data: {valu: valu},
            success: function (d) {
...
<script type="text/javascript">
 /*   function checkValidateVacationDayes() {
        if ($('#no3_agaza').val() != ' ' && $('#employee_name').val()) {
...
*/
   function checkValidateVacationDayes() {
        ...
        /*   if ($('#no3_agaza').val() != ' ' && $('#emp_code_fk').val()) {
               var actualDayes = parseFloat($('#emp_code_fk option:selected').attr('allDayes')) ;

               var casulaDayes = parseFloat($('#emp_code_fk option:selected').attr('casual_vacation')) - parseFloat($('#emp_code_fk option:selected').attr('casual'));
...
<script>
    function add_marad(value) {
        ...
        if (value != 0 && value != '' ) {
            ...
            $.ajax({
                type: 'post',
                url: '<?php echo base_url() ?>human_resources/employee_forms/all_agazat/all_orders/Vacation/insert_marad_ajax',
                data: dataString,
                dataType: 'html',
                cache: false,
...
<script>
    ...
    function update_marad(id,name) {
        var id = id;
        $('#geha_input').show();
        $('#div_add').hide();
        $('#div_update').show();
        $('#marad_title').val(name);
        $('#s_id').val(id);

       

        $('#update').on('click',function() {
            var marad_title = $('#marad_title').val();
         
            var s_id = $('#s_id').val();

            $.ajax({
                url :"<?php echo base_url() ?>human_resources/employee_forms/all_agazat/all_orders/Vacation/update_marad",
                type : "Post",
                dataType : "html",
                data: {marad_title:marad_title,id:s_id},
                success: function (html) {
                    //  alert('hh');
                    $('#marad_title').val(' ');
...
<!-------------------------------------------->
<script>
    function delte_marad(id) {
        //  confirm('هل انت متأكد من عملية الحذف ؟');
        var r =  confirm('هل انت متأكد من عملية الحذف ؟');
        if (r == true) {
            $.ajax({
                type:'post',
                url: '<?php echo base_url() ?>human_resources/employee_forms/all_agazat/all_orders/Vacation/delete_marad',
                data:{ id:id},
                dataType: 'html',
                cache:false,
                success: function(html){
